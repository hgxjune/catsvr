...
start() ->
    ...
    case application:get_env(App, tcp) of
        undefined -> ok;
        {ok, Config} ->
            ...
            {ok, _} = ranch:start_listener(?MODULE, ranch_tcp, TransOpts, tcp_echo, ProtoOpts),
            ok
    end.
...
stop() ->
    case maps:is_key(?MODULE, ranch:info()) of
        false -> ignore;
        true -> ok = ranch:stop_listener(?MODULE)
    end,
    ok.
...
get_trans_opts(Config) ->
    ...
    #{ connection_type => proplists:get_value(connection_type, Config, worker)
